...
function setGeometry(gl) {
  const centerX = 100;
  const centerY = 100;
  const outerRadius = 60;  // Bán kính ngoài của ngôi sao
  const innerRadius = 30;  // Bán kính trong của ngôi sao
  const numPoints = 8; // Số điểm của ngôi sao (cánh)

  let vertices = [];

  // Vẽ các cánh ngôi sao
  for (let i = 0; i < numPoints; i++) {
    const angle = (i / numPoints) * 2 * Math.PI;

    // Tọa độ của điểm ngoài
    const outerX = centerX + outerRadius * Math.cos(angle);
    const outerY = centerY + outerRadius * Math.sin(angle);

    // Tọa độ của điểm trong
    const innerX = centerX + innerRadius * Math.cos(angle + Math.PI / numPoints);
    const innerY = centerY + innerRadius * Math.sin(angle + Math.PI / numPoints);

    // Thêm các điểm tạo nên các cánh ngôi sao
    vertices.push(centerX, centerY); // Tọa độ trung tâm
    vertices.push(outerX, outerY);   // Điểm ngoài
    vertices.push(innerX, innerY);   // Điểm trong
  }

  // Vẽ vòng tròn trung tâm ngôi sao
  const circleRadius = 20;
  const numSegments = 30; // Độ phân giải của hình tròn
